...
[Redis Streams](https://redis.io/docs/latest/develop/data-types/streams/) is a powerful data structure that allows you to efficiently handle streams of data, similar to message queues or append-only logs.
...
- **Real-time streaming**: Redis Streams allow you to stream data in real-time to multiple consumers. _In this case, user can see the output in real-time as it is generated instead of waiting for the entire LLM output to be generated._
...
git clone https://github.com/PrasanKumar93/redis-short-demos.git
...
![stream llm output](./images/stream-llm-output.gif)
...
Note : In this tutorial, we are using `xRead` command to read the stream data, but you can also use `xReadGroup` command to read the stream data in consumer groups and handle the consumer acknowledgment and re-reading of the stream data in case of failure.
...
Let's monitor the Redis stream created by the application using RedisInsight.

![RI Streams](./images/ri-streams.png)

Also visualize the `question` JSON stored in Redis

![RI JSON](./images/ri-json.png)
