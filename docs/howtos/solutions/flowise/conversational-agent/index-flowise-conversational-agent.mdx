...
In addition to `chat memory` in Flowise, you can also use Redis as `vector store` to store and retrieve embeddings for your conversational agent. This can be useful for tasks like **semantic search**, **recommendation systems**, and more.

You can try advanced example by [importing this chatflow](./TextFileRAGSearchChatflow.json) in Flowise and configuring the Redis and OpenAI nodes. Upload some [sample products](./products.zip) or other text files in the `Text File` node.
This sample chatflow demonstrates how to use Redis as a vector store to perform semantic search using the RAG (Retrieval-Augmented Generation) concept.
